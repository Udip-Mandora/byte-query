...
export async function answerGetOneById(id: string): Promise<{
    ...
    const answer = await db.query.AnswerTable.findFirst({ where: eq(AnswerTable.id, id) });
    return answer;
}
...
export async function answerGetOneByQuestionId(questionId: string): Promise<{
    ...
    const answer = await db.query.AnswerTable.findFirst({ where: eq(AnswerTable.questionId, questionId) });
    return answer;
}
...
export async function answerGetOneByUserId(userId: string): Promise<{
    ...
    const answer = await db.query.AnswerTable.findFirst({ where: eq(AnswerTable.userId, userId) });
    return answer;
}
 ...
 */
export async function answerCreateOne(data: { content: string, userId: string, questionId: string, upVote: number, downVote: number }): Promise<{ id: string }> {
